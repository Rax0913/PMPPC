...
// #include "config.hpp"
// #include "aero_get_corridor/glbmap.hpp"
...
private:
    ...
    inline void set_param(Eigen::Vector3d mapLb,Eigen::Vector3d mapUb, double rel)
    {
        astar_finder->setHweight(3.0);
        roofheight = mapUb(2)-0.5;// - 0.1;
        floorheight = mapLb(2);// + 0.1;
        _resolution = rel;
        _x_l = mapLb(0);
        _x_h = mapUb(0);
        _y_l = mapLb(1);
        _y_h = mapUb(1);
        _z_l = std::max(mapLb(2), floorheight);
        _z_h = std::min(mapUb(2), roofheight);
...
public:
    ...
    inline void astarInit(const Eigen::Vector3d maplb,
                const Eigen::Vector3d mapub, double rel)
    {
            this->set_param(maplb,mapub,rel);
            this->ros_init();
